...
render data — it may never *decide* a price, a loyalty point value, a stock
...
| Async jobs | **BullMQ (Redis-backed)** | Notifications, receipt generation, loyalty point accrual, and inventory deduction happen *after* the order response, not blocking checkout latency | AWS SQS (viable later; BullMQ is zero extra infra now) |
